...
- [ ] Create a new user from `firstName`, `lastName`, `email`; return the new user ID
- [ ] All users returned by the service are converted to DTOs that:
...
- [ ] **Framework-agnostic** — must drop into Laravel, Drupal, WordPress, or vanilla PHP with no framework coupling
- [ ] Target **PHP 8.4**, strict types, modern PSR standards (PSR-4, PSR-12)
- [ ] **PHPStan** static analysis (ideally at `max` / level 9)
- [ ] **Unit tests** for the service; mocked transport so tests pass offline and are deterministic
- [ ] Optional integration/API tests separate from the default test run
- [ ] Clean exception handling: generic transport/API errors surfaced as **domain-specific exceptions** with useful context for consumers
- [ ] No reinvented wheels — use established standalone packages where sensible (no full frameworks)
...
These need answers (or explicit "defer to defaults") before I start implementing.
...
1. Work through the questions above and lock in decisions.
2. Scaffold `composer.json`, namespace, CI, PHPStan, PHPUnit.
3. Build inner → outer: DTOs → exceptions → client → service → tests.
4. Wire up integration test suite (opt-in).
5. Polish README with usage examples once the public API is stable.
